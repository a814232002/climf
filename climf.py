...
def update(data,U,V,lbda,gamma):
	...
	for i in range(len(U)):
		...
		f = precompute_f(data,U,V,i)
		for j in f:
			dU += (dg(f[j])/g(f[j]))*V[j]-lbda*U[i]
			for k in f:
				dU += (V[j]-V[k])*dg(f[k]-f[j])/(1-g(f[k]-f[j]))
			dV = (dg(f[j])/g(f[j]))*U[i]-lbda*V[j]
			for k in f:
				dV += (dg(f[k]-f[j])/(1-g(f[k]-f[j])))*U[i]
			V[j] += gamma*dV
		U[i] += gamma*dU
